...
package object systemloader {
  ...
  def linuxStartScriptUrl(sourceDirectory: File,
    ...
    val loader = loaderOpt.getOrElse(
      sys.error(
        "No serverLoader defined. Enable a systemloader, e.g. with `enablePlugins(UpstartPlugin)`")
    )
    overrideFromFile(sourceDirectory, loader, name).getOrElse(
      getClass getResource in(loader, name))
  }

  def loaderFunctionsReplacement(
      sourceDirectory: File,
      loaderOpt: Option[ServerLoader]): (String, String) = {
    val replacement = for {
      loader <- loaderOpt
    ...
    replacement.getOrElse(
      sys.error(s"Loader functions could not be loaded for ${loaderOpt}"))
  }
  ...
  def startScriptMapping(
    ...
    for {
      ...
      LinuxPackageMapping(
        Seq(s -> path),
        LinuxFileMetaData(Users.Root, Users.Root, perms, isConf.toString))
  }

  private def in(loader: ServerLoader, name: String): String =
    loader.toString + "/" + name

  private def overrideFromFile(sourceDirectory: File,
                               loader: ServerLoader,
                               name: String): Option[URL] = {

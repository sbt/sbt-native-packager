...
object JavaServerAppPackaging extends AutoPlugin {
  ...
  def linuxSettings: Seq[Setting[_]] = Seq(
    ...
    linuxPackageMappings <+= (packageName in Linux,
                              ...
                              daemonUser in Linux,
                              daemonGroup in Linux) map {
      (name, logsDir, user, group) =>
    ...
    linuxPackageSymlinks <+= (packageName in Linux,
                              ...
                              defaultLinuxLogsLocation) map {
      (name, install, logsDir) =>
        LinuxSymlink(install + "/" + name + "/logs", logsDir + "/" + name)
    },
    // === etc config mapping ===
    bashScriptEnvConfigLocation := Some(
      "/etc/default/" + (packageName in Linux).value),
  ...
  def debianSettings: Seq[Setting[_]] = {
    ...
    inConfig(Debian)(etcDefaultConfig) ++
      inConfig(Debian)(
        Seq(
          // === Extra replacements ===
          linuxScriptReplacements ++= bashScriptEnvConfigLocation.value
            .map(ENV_CONFIG_REPLACEMENT -> _)
            .toSeq,
          linuxScriptReplacements += Names.DaemonStdoutLogFileReplacement -> daemonStdoutLogFile.value
            .getOrElse(""),
          // === Maintainer scripts ===
          maintainerScripts := {
            ...
            scripts ++ Map(
              Preinst -> (scripts.getOrElse(Preinst, Nil) ++ contentOf(
                Preinst)),
              Postinst -> (scripts.getOrElse(Postinst, Nil) ++ contentOf(
                Postinst)),
    ...
    inConfig(Rpm)(etcDefaultConfig) ++
      inConfig(Rpm)(
        Seq(
          ...
          linuxPackageMappings <+= (packageName, daemonUser, daemonGroup) map {
            (name, user, group) =>
   ...
   * - Provided template
   */
  private[this] def getEtcTemplateSource(
      sourceDirectory: File,
      loader: Option[ServerLoader]): java.net.URL = {
  ...
  protected def etcDefaultMapping(
      conf: Option[File],
      envLocation: Option[String]): Seq[LinuxPackageMapping] = {
    val mapping = for (path <- envLocation;
                       c <- conf)
      yield
        LinuxPackageMapping(
          Seq(c -> path),
          LinuxFileMetaData(Users.Root, Users.Root, "644")).withConfig()
  ...
  /**
    ...
    * @return script lines
    */
  private[this] def getScriptContent(
  ...
  protected def rpmScriptletContents(
    ...
    val predefinedScripts = predefined.foldLeft(scripts) {
      case (scripts, script) =>
        ...
        // generate content
        val content = JavaServerBashScript(script,
                                           ARCHETYPE,
                                           Rpm,
                                           replacements,
                                           userDefined).map { script =>
    ...
    rpmScripts
      .filter(s => s.isFile && !predefined.contains(s.getName))
      .foldLeft(predefinedScripts) {
        case (scripts, scriptlet) =>
          val script = scriptlet.getName
          val existingContent = scripts.getOrElse(script, Nil)

          val loadedContent =
            JavaServerBashScript(script,
                                 ARCHETYPE,
                                 Rpm,
                                 replacements,
                                 Some(scriptlet.toURI.toURL)).map { script =>
              TemplateWriter generateScriptFromString (script, replacements)
            }.toSeq

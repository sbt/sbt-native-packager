...
trait DockerKeys {
    ...
    val dockerRmiCommand = SettingKey[Seq[String]]("dockerRmiCommand", "Command for removing the Docker image from the local registry")
    val dockerDocfileCommands = SettingKey[Seq[CmdLike]]("dockerDocFileCommands", "Commands for including in Dockerfile")

...
object LauncherJarPlugin extends AutoPlugin {
  ...
  override lazy val projectSettings = Defaults.packageTaskSettings(
      packageJavaLauncherJar,
      mappings in packageJavaLauncherJar) ++ Seq(
      ...
      val manifestMainClass =
        (mainClass in (Compile, packageJavaLauncherJar)).value
          .map(Attributes.Name.MAIN_CLASS -> _)
      Seq(
        ManifestAttributes(
      ...
      artifactName in packageJavaLauncherJar := {
      (scalaVersion, moduleId, artifact) =>
      ...
      Some(
        "-jar $lib_dir/" + (artifactPath in packageJavaLauncherJar).value.getName)
    },
      scriptClasspath in bashScriptDefines := Nil,
      mainClass in (Compile, batScriptReplacements) := {
      Some(
        "-jar %APP_LIB_DIR%\\" + (artifactPath in packageJavaLauncherJar).value.getName)
    },

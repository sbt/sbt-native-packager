...
TaskKey[Unit]("check-spec-file") <<= (target, streams) map { (target, out) =>
  ...
  assert(spec contains "%attr(0755,root,root) /etc/init.d/rpm-test",
         "Wrong /etc/init.d/\n" + spec)
  assert(spec contains "%config %attr(644,root,root) /etc/default/rpm-test",
         "Wrong etc default file\n" + spec)
  assert(spec contains "%dir %attr(755,rpm-test,rpm-test) /var/log/rpm-test",
         "Wrong logging dir path\n" + spec)
  assert(spec contains "%dir %attr(755,rpm-test,rpm-test) /var/run/rpm-test",
...
TaskKey[Unit]("unzip") <<= (packageBin in Rpm, streams) map {
  (rpmFile, streams) =>
    ...
    Process("rpm2cpio", rpmPath) #| Process("cpio -i --make-directories") ! streams
      .log
    ()
}

TaskKey[Unit]("checkStartupScript") <<= (target, streams) map {
  (target, out) =>

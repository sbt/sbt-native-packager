...
TaskKey[Unit]("check-spec-file") <<= (target, streams) map { (target, out) =>
  ...
  assert(spec contains "%attr(0755,root,root) /etc/init.d/rpm-package",
         "Wrong /etc/init.d path\n" + spec)
  assert(spec contains "%config %attr(644,root,root) /etc/default/rpm-package",

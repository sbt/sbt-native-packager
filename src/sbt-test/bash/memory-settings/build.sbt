...
TaskKey[Unit]("jvmopts-check") := {
  ...
  assert(options contains "-J-Xmx64m",
         "Options don't contain xmx setting:\n" + options)
  assert(options contains "-J-Xms64m",
         "Options don't contain xms setting:\n" + options)
}
...
TaskKey[Unit]("run-check") := {
  ...
  assert(memory.toLong <= 64,
         "Runtime memory is bigger then 64m < " + memory + "m")
}

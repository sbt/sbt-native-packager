...
TaskKey[Unit]("check-classspath") := {
  ...
  assert(
    bat contains "set \"APP_CLASSPATH=%APP_LIB_DIR%\\classpath-jar-test.classpath-jar-test-0.1.0-classpath.jar\"")
  val jar = new java.util.jar.JarFile(
    dir / "lib" / "classpath-jar-test.classpath-jar-test-0.1.0-classpath.jar")
  assert(
    jar
      .getManifest()
      .getMainAttributes()

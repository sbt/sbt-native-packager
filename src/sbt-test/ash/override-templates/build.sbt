...
ashScriptReplacements := Seq("mainclass" -> "FOO", "available-mainclasses" -> "BRAVO CHARLIE", "classpath" -> "foo.jar:bar.jar")

TaskKey[Unit]("runCheck") := {
  val cwd = (stagingDirectory in Universal).value / "bin"
  val source = scala.io.Source.fromFile(cwd / packageName.value + ".sh")
  ...
  assert(contents.contains("MAINCLASS=FOO"), "Template didn't contain MAINCLASS: \n" + contents)

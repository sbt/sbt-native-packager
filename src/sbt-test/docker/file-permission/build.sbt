  ...
  .settings(
    ...
    checkDockerfileDefaults := {
      ...
      assertEquals(
        ...
        """WORKDIR /opt/docker
          ...
          |COPY 40/opt /40/opt
          |USER root
          |RUN ["chmod", "-R", "u=rX,g=rX", "/20/opt/docker"]
          |RUN ["chmod", "-R", "u=rX,g=rX", "/40/opt/docker"]
          |RUN ["chmod", "u+x,g+x", "/40/opt/docker/bin/file-permission-test"]
          |
    ...
    checkDockerfileWithStrategyCopyChown := {
      ...
      assertEquals(
        ...
        """FROM fabric8/java-centos-openjdk8-jdk AS mainstage
          |WORKDIR /opt/docker
          |COPY --chown=daemon:root 20/opt /opt
          |COPY --chown=daemon:root 40/opt /opt
          |USER daemon
          |ENTRYPOINT ["/opt/docker/bin/file-permission-test"]
          |CMD []""".stripMargin.linesIterator.toList
      )
    ...
    checkDockerfileWithWriteExecute := {
      ...
      assertEquals(
        ...
        """WORKDIR /opt/docker
          ...
          |COPY 40/opt /40/opt
          |USER root
          |RUN ["chmod", "-R", "u=rwX,g=rwX", "/20/opt/docker"]
          |RUN ["chmod", "-R", "u=rwX,g=rwX", "/40/opt/docker"]
          |RUN ["chmod", "u+x,g+x", "/40/opt/docker/bin/file-permission-test"]
          |
          |FROM fabric8/java-centos-openjdk8-jdk AS mainstage
          |USER root

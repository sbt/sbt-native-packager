...
TaskKey[Unit]("check-cygwin-script") <<= (stagingDirectory in Universal,
                                          name,
                                          streams) map {
  (dir, name, streams) =>
    ...
    if (!cygwinBash.exists)
      sys.error(
        "Unable to find the default cygwin (c:\\cygwin) install for testing.")
    else {
      val script = dir / "bin" / name
      val PathParts = "([\\w])+\\:\\\\(.+)".r
      val PathParts(drive, path) = script.getAbsolutePath
      val cygdriveScriptPath = "/cygdrive/" + drive.toLowerCase + "/" + path
          .replaceAll("\\\\", "/")
      ...
      val result =
        Process(cmd, Some(dir), "PATH" -> pathEnv) ! streams.log match {
          ...
          case n =>
            sys.error(
              "Failed to run script: " + cygdriveScriptPath + " error code: " + n)
        }

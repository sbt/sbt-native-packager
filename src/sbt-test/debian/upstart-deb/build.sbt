...
TaskKey[Unit]("check-control-files") <<= (target, streams) map {
  (target, out) =>
    ...
    assert(postinst contains "initctl reload-configuration",
           "postinst misses initctl: " + postinst)
    assert(postinst contains """startService debian-test""",
           "postinst misses service start: " + postinst)
    assert(prerm contains """stopService debian-test""",
           "prerm misses stop: " + prerm)
...
TaskKey[Unit]("check-startup-script") <<= (target, streams) map {
  (target, out) =>
    val script = IO.read(
      target / "debian-test-0.1.0" / "etc" / "init" / "debian-test.conf")
    assert(script.contains("start on runlevel [2345]"),
           "script doesn't contain start on runlevel header\n" + script)
    assert(script.contains("stop on runlevel [016]"),
           "script doesn't contain stop on runlevel header\n" + script)
    assert(!script.contains("start on started"),
           "script contains start on started header\n" + script)
    assert(!script.contains("stop on stopping"),
           "script contains stop on stopping header\n" + script)
    // should contain
    assert(

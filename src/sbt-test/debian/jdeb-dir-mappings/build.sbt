...
TaskKey[Unit]("check-dir-mappings") <<= (target, streams) map {
  (target, out) =>
//  val tmpDir = java.nio.file.Files.createTempDirectory("jdeb")
    ...
    assert((extracted / "usr/share/conf/application.conf").exists(),
           "File application.conf not exists")
    assert((extracted / "usr/share/conf/log4j.properties").exists(),
           "File log4j.properties not exists")
